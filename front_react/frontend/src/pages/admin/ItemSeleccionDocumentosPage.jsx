...
export default function ItemSeleccionDocumentosPage() {
  const { id } = useParams()
    ...
    const cargarDocumentos = async () => {
      try {
        ...
        const [itemRes, relacionesRes] = await Promise.all([
          api.get(`/items/seleccion-unica/${id}/`),
          api.get("/items/seleccion-unica-documentos/")
        ])
  ...
  return (
    <div className="bg-gradient">
      <div className="container">
        <AdminPageHeader
          title="Documentos de Selección Única"
          subtitle="Previsualiza los archivos asociados al ítem."
        />

        <div className="card">
          <h2>Documentos del item de seleccion unica #{id}</h2>
